      ...
      <div class="checkout-steps">
        ...
        <div class="submit">
          <!-- <router-link class="btn"
                       to="/paysuccess">立即支付</router-link> -->
          <a href="javascript:;"
             class="btn"
             @click="btnPayOpen">立即支付</a>
        </div>
        <div class="otherpay">
          <div class="step-tit">
            <h5>其他支付方式</h5>
...
export default {
  ...
  mounted () {
    this.$store.dispatch('getPayInfo', this.orderId).then(() => {
    }).catch(err => {
      err
    })
  },
      ...
      try {
        ...
        if (!this.timer) {
          this.timer = setInterval(() => {
            this.$store.dispatch('getPayStatus', this.orderId).then((res) => {
              if (res.code === 200) {
