      ...
      <div class="cart-body">
        <ul class="cart-list"
          ...
          <li class="cart-list-con7">
            <a href="#none"
               class="sindelet"
               @click="deleteCartById(cart.skuId)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    changeChecked (skuId, event) {
      ...
      this.$store.dispatch('changeCheckedById', { skuId, isChecked: checked }).then(res => this.getData())
    },
    deleteAllCheckedCart () {
      this.$store.dispatch('deleteCartCheckedCart').then(res => {
        this.getData()
      }).catch(err => err)
    },
    changeAllCartChecked (event) {
      let checked = event.target.checked ? '1' : '0'
      this.$store.dispatch('changeAllCartChecked', checked).then(res => {
        this.getData()
      })
        .catch(err => err)
    }
