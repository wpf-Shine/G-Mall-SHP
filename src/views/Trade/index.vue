<template>
  <div class="trade-container"
       v-if="Object.keys(addressInfo).length>0">
    ...
    <div class="content">
      ...
      <div class="address clearFix">
        <span class="username"
              :class="{selected:paymentWay =='ONLINE' ? true:false}"
              @click="changePay('ONLINE')">在线支付</span>
        <span class="username"
              style="margin-left:5px;"
              :class="{selected:paymentWay =='OFFLINE' ? true:false}"
              @click="changeArrival('OFFLINE')">货到付款</span>
...
export default {
  ...
  methods: {
    ...
    changePay (paymentWay) {
      this.paymentWay = paymentWay
    },
    changeArrival (paymentWay) {
      this.paymentWay = paymentWay
    },
    async submitOrder () {
      ...
      try {
        ...
        this.$router.push('/pay?orderId=' + this.$store.state.trade.orderId)
      }
      catch (err) {
        alert(err)
      }
